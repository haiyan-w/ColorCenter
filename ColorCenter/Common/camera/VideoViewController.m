...
{
    BOOL cameraAuthorized  = YES;
    AVAuthorizationStatus status = [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo];
    if ((status == AVAuthorizationStatusDenied)||(status == AVAuthorizationStatusRestricted)) {
        cameraAuthorized  = NO;
    }
    
    BOOL photoAuthorized  = YES;
    PHAuthorizationStatus status2 = [PHPhotoLibrary authorizationStatus];
    if (status2 == PHAuthorizationStatusRestricted ||
        status2 == PHAuthorizationStatusDenied) {
        photoAuthorized  = NO;
    }
    
    
    if (!cameraAuthorized || !photoAuthorized) {
        AlertAction * yes = [[AlertAction alloc] initWithTitle:@"去开启" action:^(id _Nonnull arg) {
        ...
        AlertAction * no = [[AlertAction alloc] initWithTitle:@"暂不开启" action:^(id _Nonnull arg) {
            __strong typeof(self) strongSelf = weakSelf;
            [strongSelf onCancelAction:nil];
        }];

        AlertViewController * alert = [[AlertViewController alloc] initWithTitle:@"开启相机并允许访问相册才能拍摄哟" message:@"" position:AlertCenter actions:@[no,yes]];
        [alert showOn:self];
    }
}
...
//                }
////                if (self.delegate && [self.delegate respondsToSelector:@selector(videoViewController:takeVideo:)]) {
////                    [self.delegate videoViewController:self takeVideo:assetURL];
////                }
//                NSLog(@"成功保存视频到相簿.");
...
//
////        UIImageWriteToSavedPhotosAlbum(self.takeImage, self, nil, nil);
////        if (self.delegate && [self.delegate respondsToSelector:@selector(videoViewController:takeImage:)]) {
////            [self.delegate videoViewController:self takeImage:self.takeImage];
////        }
////
////        [self onCancelAction:nil];
//    }
...
- (IBAction)onEnsureAction:(UIButton *)sender {
    ...
    if (authorStatus == PHAuthorizationStatusNotDetermined){
        //首次访问，弹窗请求访问相册，允许后获取最近照片
        __weak typeof(self) weakSelf = self;
        [self requestAuthorizationWithCompletion:^{
            __strong typeof(self) strongSelf = weakSelf;
            [strongSelf saveToSend];
        }];
...
//        [Utility showProgressDialogText:@"视频处理中..."];
        ...
        __block NSString *createdAssetID =nil;//唯一标识，可以用于图片资源获取
            ...
            [[PHPhotoLibrary sharedPhotoLibrary] performChangesAndWait:^{
                createdAssetID = [PHAssetChangeRequest        creationRequestForAssetFromVideoAtFileURL:self.saveVideoUrl].placeholderForCreatedAsset.localIdentifier;
            } error:&error];
        ...
        //                [Utility showAllTextDialog:KAppDelegate.window Text:@"保存视频到相册发生错误"];
                        ...
                        PHFetchResult<PHAsset *> * result = [PHAsset fetchAssetsWithLocalIdentifiers:@[createdAssetID] options:nil];
                        PHAsset * asset = [result firstObject];
                        
                        if (weakSelf.takeBlock) {
                            weakSelf.takeBlock(asset);
                        }
                        NSLog(@"成功保存视频到相簿.");
                        [weakSelf onCancelAction:nil];
                    }
        ...
        __block NSString *createdAssetID =nil;//唯一标识，可以用于图片资源获取
            NSError *error =nil;
            [[PHPhotoLibrary sharedPhotoLibrary] performChangesAndWait:^{
                createdAssetID = [PHAssetChangeRequest creationRequestForAssetFromImage:self.takeImage].placeholderForCreatedAsset.localIdentifier;
            } error:&error];
