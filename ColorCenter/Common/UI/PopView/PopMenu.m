...
@property (nonatomic, readwrite, copy)NSArray * menuIcons;
...
- (instancetype)initWithArrow:(CGPoint)point menuSize:(CGSize)size arrowStyle:(CWPopMenuArrowPosition)position menuStrings:(NSArray *)strings icons:(NSArray *)icons
...
{
    self.bgAlpha = 0.2;
    self.menuViewBgColor = [UIColor blackColor];
    self.menuRadius = 4.f;
//    self.arrowWidth = 15.f;
//    self.arrowHeight = 10.f;
    self.arrowWidth = 8.f;
    self.arrowHeight = 5.f;
    self.cellHeight = 46.f;
...
//    [self.layer addSublayer:_shLayer];
    ...
    CGFloat deltaX = (peak.x - origin.x)*_arrowRadius/(origin.y - peak.y);
//
        CGPoint pt1  = CGPointMake(peak.x - deltaX, peak.y + _arrowRadius);
        CGPoint pt2  = CGPointMake(peak.x + deltaX, peak.y + _arrowRadius);
        [path addLineToPoint:pt1];
    
//        [path addArcWithCenter:CGPointMake(peak.x, peak.y + _arrowRadius) radius:_arrowRadius startAngle:0 endAngle:180 clockwise:NO];
//    [path moveToPoint:pt1];
    [path addQuadCurveToPoint:pt2 controlPoint:peak];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.menuIcons) {
        cell.icon.image = [UIImage imageNamed:[self.menuIcons objectAtIndex:indexPath.row]];
        cell.iconWidthConstraint.constant = 16;
        cell.spaceConstaint.constant = 8;
    }else {
        cell.iconWidthConstraint.constant = 0;
        cell.spaceConstaint.constant = 0;
    }
